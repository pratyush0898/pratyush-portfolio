...
export async function GET() {
  try {
    ...
    if (isCacheValid()) {
      ...
      await sleep(5000); // wait 5 seconds before sending cached response
    ...
    console.log(
      `✨ Cached ${
        articles.length
      } articles at ${new Date().toLocaleTimeString()}`
    );

    await sleep(5000); // wait 5 seconds before sending fresh response
